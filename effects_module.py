...
class Coin_spark_effects():
	num_of_imgs = 9
	imgs_small = [pygame.image.load(os.path.join('Utils/Pics/Coins/Effects/',str(x)+'.png')).convert_alpha() for x in range(num_of_imgs)]
	...
	def draw(self, win):
		self.x -= fg_module.foreground_speed
