...
def change_img_pixel_format():
	...
	obstacles_module.Tree.imgs = [img.convert_alpha() for img in obstacles_module.Tree.imgs]	
	...
	shark_module.Shark.list_of_lists = [[img.convert_alpha() for img in lst] for lst in shark_module.Shark.list_of_lists]

def draw_all_objects():
	"""
	Draws the background, foreground, obstacles, coins, special effects, player, dragon, ghost, shark, lives, minimap.
	"""
...
def main(volume_button_on_status):
	...
	#Music Variable
	pygame.mixer.music.load(os.path.join(r'level_1\Utils\Music\BGmusic_Level3.wav'))
	...
	while run:
		...
		collision_with_dragon = dragon_module.collision_with_dragon()
		if collision_with_obstacle or collision_with_dragon:		# Dummy exit
			if volume_button_on_status:
